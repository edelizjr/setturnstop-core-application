...
@RestController
@RequestMapping("/api/setturnstop-kafka")
public class SetturnstopKafkaResource {

    private final Logger log = LoggerFactory.getLogger(SetturnstopKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public SetturnstopKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
