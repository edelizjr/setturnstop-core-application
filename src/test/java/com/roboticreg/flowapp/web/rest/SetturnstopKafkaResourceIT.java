...
import com.roboticreg.flowapp.config.KafkaSseConsumer;
import com.roboticreg.flowapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SetturnstopKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/setturnstop-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/setturnstop-kafka/consume")
